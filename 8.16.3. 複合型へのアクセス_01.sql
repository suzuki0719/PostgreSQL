...
create function f(inventory_items) returns inventory_items as $$
    select $1.name,$1.supplier_id,$1.price;
; $$ LANGUAGE SQL;

--複合型の列にアクセスする場合は()で複合型をかこう必要がある
select (f(item)).name from on_hand;
